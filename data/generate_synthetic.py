"""
KubeSage Incident Generator
===========================
A utility to generate a synthetic dataset of 500+ realistic Kubernetes 
incident post-mortems (OOMKilled, CrashLoopBackOff, DNS failures, etc.). 
Used for testing and calibrating the vector retrieval pipeline.
"""
...
INCIDENT_TEMPLATES: list[dict[str, Any]] = [
    # ── OOMKilled ──
    {
        "type": "OOMKilled",
        "title_templates": [
            "{service} pod {pod} OOMKilled in namespace {namespace}",
            "Memory limit exceeded: {service} pod {pod} terminated",
            "Out of memory: {service} containers killed by OOM killer",
        ],
        "description_template": (
            "Pod {pod} in namespace {namespace} was terminated with reason OOMKilled. "
            "The container exceeded its memory limit of {memory_limit}. "
        ...
        "log_patterns": [
            ...
            "Container {container_id} was OOMKilled",
            "oom-kill:constraint=CONSTRAINT_MEMCG",
        ],
        "root_causes": [
            "Memory leak in application code — unbounded data structure growth",
            "Insufficient memory limits configured for workload",
            "Sudden traffic spike causing increased memory allocation",
            "Third-party library memory leak under concurrent requests",
            "Unoptimized database query loading entire result set into memory",
        ],
        "resolutions": [
            ...
            "Optimized query to use cursor-based pagination",
            "Implemented circuit breaker to shed load during spikes",
        ],
    ...
    # ── CrashLoopBackOff ──
    {
        ...
        "title_templates": [
            ...
            "Repeated crashes detected: {service} pod {pod}",
            "Container restart loop: {service} failing to start",
        ],
        "description_template": (
            ...
            "Error in container logs: '{error_message}'. "
            "The pod has restarted {restart_count} times in the last 30 minutes."
        ),
        "log_patterns": [
            "Error: {error_message}",
        ...
        "root_causes": [
            ...
            "Failed database connection on startup",
            "Invalid container entrypoint or command",
            "Port conflict with host or sidecar container",
        ],
        "resolutions": [
            "Fixed environment variable configuration in ConfigMap",
            "Created missing Secret '{secret_name}' for database credentials",
            "Added startup probe with 30s initial delay",
            "Corrected container command to use proper entrypoint",
    ...
    # ── ImagePullBackOff ──
    {
        ...
        "title_templates": [
            ...
            "Cannot pull container image: {service} pod {pod}",
            "Image pull error: {image_name} not found or unauthorized",
        ],
        "description_template": (
            "Pod {pod} in namespace {namespace} is stuck in ImagePullBackOff. "
            "Failed to pull image '{image_name}' from registry {registry}. "
            "Error: '{pull_error}'. The deployment was triggered {time_ago} minutes ago."
        ),
        "log_patterns": [
            "Failed to pull image \"{image_name}\": rpc error: code = {code}",
            "ErrImagePull: {pull_error_detail}",
            "Error response from daemon: pull access denied for {image_name}",
            "manifest for {image_name} not found",
        ],
        "root_causes": [
            "Incorrect image tag specified in deployment manifest",
            "Registry authentication failure — missing imagePullSecret",
            "Docker registry rate limit exceeded",
            "Image does not exist in the specified registry",
            "Network connectivity issue to container registry",
        ],
        "resolutions": [
            "Updated image tag from '{old_tag}' to '{new_tag}'",
            "Created imagePullSecret '{secret_name}' and referenced in ServiceAccount",
            "Switched to local registry mirror to avoid rate limits",
            "Verified and corrected image path in deployment YAML",
    ...
    # ── Connection Pool Exhaustion ──
    {
        ...
        "title_templates": [
            ...
            "{service} failing: PostgreSQL max_connections reached",
            "Connection timeout errors in {service} — pool starvation",
        ],
        "description_template": (
            "{service} is experiencing connection pool exhaustion. "
            "All {pool_size} connections in the pool are active. "
            "{waiting_count} requests are waiting for connections. "
            "Average wait time: {avg_wait}ms. "
            "Database {db_name} shows max_connections = {max_conn} reached."
        ),
        "log_patterns": [
            "FATAL: sorry, too many clients already",
            "Timeout waiting for idle connection: pool exhausted",
            "Cannot acquire connection from pool within {timeout}s",
            "Connection pool size {pool_size} exceeded for {db_name}",
        ],
        "root_causes": [
            "Connection leak — connections not returned to pool after use",
            "Sudden traffic spike to service causing pool saturation",
            "Slow database queries causing connections to be held open",
            "Pool size misconfiguration — pool too small for workload",
        ...
        "resolutions": [
            ...
            "Optimized slow queries — 10x improvement in response time",
        ...
        "sev_weights": [0.6, 0.3, 0.1],
    },
    # ── DNS Failures ──
    {
        "type": "DNSFailure",
        "title_templates": [
            "DNS resolution failures in {service} pod {pod}",
            "Service discovery broken: {service} cannot resolve hostnames",
        ...
        "description_template": (
            ...
            "nslookup returns: '{dns_error}'. "
            "CoreDNS pods in kube-system are reporting {core_dns_status}."
        ),
        "log_patterns": [
            "lookup {hostname} on {dns_server}: no such host",
            "dial tcp: lookup {hostname}: Temporary failure in name resolution",
            "getaddrinfo EAI_AGAIN {hostname}",
            "plugin/errors: 2 {hostname}. A: read udp: i/o timeout",
        ],
        "root_causes": [
            "CoreDNS pods overloaded due to high query volume",
            "NetworkPolicy blocking DNS traffic (UDP port 53)",
            "CoreDNS ConfigMap misconfiguration",
            "Node-level DNS cache corruption",
            "NDots configuration causing excessive DNS lookups",
        ],
        "resolutions": [
            "Scaled CoreDNS deployment to {replicas} replicas",
            "Updated NetworkPolicy to allow UDP 53 traffic to kube-dns",
            "Fixed CoreDNS ConfigMap — added missing stub domain",
            "Flushed node DNS cache and restarted CoreDNS pods",
            "Set ndots:2 in pod spec to reduce DNS query volume",
        ],
        "services": ["all-services", "api-gateway", "service-mesh"],
        "severities": ["Critical", "High", "High", "Medium"],
        "sev_weights": [0.45, 0.30, 0.15, 0.10],
    },
    # ── CPU Throttling ──
    {
        "type": "CPUThrottling",
        "title_templates": [
            "CPU throttling detected for {service} pod {pod}",
            "{service} experiencing high latency due to CPU limits",
            "Container CPU throttled: {service} exceeding limits",
        ],
        "description_template": (
            "Pod {pod} in namespace {namespace} is being CPU throttled. "
            "CPU usage regularly exceeds limit of {cpu_limit}. "
            "Throttling rate: {throttle_pct}% over the last 5 minutes. "
            "P99 latency increased from {old_latency}ms to {new_latency}ms."
        ),
        "log_patterns": [
            "Container {container_id} is experiencing cpu throttling",
            "CPU usage {usage_pct}% exceeding threshold",
            "Performance degradation due to CPU limit enforcement",
            "Warning: cpu.cfs_period_us throttling triggered",
        ],
        "root_causes": [
            "CPU limits set too low for actual workload",
            "Inefficient code path consuming excessive CPU under load",
            "Missing HorizontalPodAutoscaler for variable workload",
            "Sidecar container consuming unexpected CPU resources",
            "GC (garbage collection) overhead in JVM-based service",
        ],
        "resolutions": [
            "Increased CPU limit from {old_limit} to {new_limit} cores",
            "Optimized hot code path — 40% reduction in CPU usage",
            "Configured HPA with CPU utilization target of 70%",
            "Reduced sidecar CPU request and added resource quotas",
            "Tuned JVM GC settings (G1GC, MaxGCPauseMillis=200)",
        ],
    ...
    # ── Network Failures ──
    {
        "type": "NetworkFailure",
        "title_templates": [
            "Network connectivity loss in {service} pod {pod}",
            "Packet loss between {service} and {target_service}",
            "Network timeout: {service} cannot reach external API",
        ],
        "description_template": (
            "Pod {pod} in namespace {namespace} is experiencing network failures. "
            "Cannot connect to {target_service} at {target_host}:{target_port}. "
            "Error: '{network_error}'. "
            "Packet loss: {packet_loss}%. Latency: {latency}ms (normal: {normal_latency}ms)."
        ),
        "log_patterns": [
            "dial tcp {target_host}:{target_port}: i/o timeout",
            "connect: connection refused",
            "read tcp: connection reset by peer",
            "net/http: request canceled while waiting for connection",
        ],
        "root_causes": [
            "NetworkPolicy blocking egress traffic to required service",
            "Service mesh (Istio/Linkerd) sidecar proxy misconfiguration",
            "Network partition between node groups",
            "Firewall rule blocking port {target_port}",
            "MTU mismatch causing packet fragmentation",
        ],
        "resolutions": [
            "Updated NetworkPolicy to allow egress to {target_host}:{target_port}",
            "Restarted Istio sidecar proxy and verified Envoy configuration",
            "Resolved network partition by fixing VPC peering route tables",
            "Added firewall rule allowing port {target_port} outbound",
...
def fill_template(template: str, **kwargs: Any) -> str:
    ...
    try:
        ...
        # Fallback: just return template as-is
        return template
...
def generate_incident(index: int) -> dict[str, Any]:
    ...
    ctx = {
        ...
        "db_name": db_name,
        "max_conn": max_conn,
        "waiting_count": random.randint(5, 50),
        "avg_wait": random.randint(100, 5000),
        "cpu_limit": cpu_limit,
        "throttle_pct": random.randint(10, 95),
        "old_latency": old_latency_ms,
        "new_latency": new_latency_ms,
        "hostname": f"{service}.{namespace}.svc.cluster.local",
        ...
        "packet_loss": random.randint(5, 80),
        "latency": random.randint(500, 5000),
        "normal_latency": random.randint(1, 50),
        "old_limit": random.choice(["128Mi", "256Mi", "0.5", "1.0"]),
        "new_limit": random.choice(["512Mi", "1024Mi", "2.0", "4.0"]),
        "old_pool": str(pool_size),
        "new_pool": str(pool_size * random.choice([2, 3, 4, 5])),
        "old_tag": f"v{random.randint(1,3)}.{random.randint(0,3)}.{random.randint(0,9)}",
        "new_tag": f"v{random.randint(4,5)}.{random.randint(0,5)}.{random.randint(0,9)}",
        "secret_name": f"{service}-registry-creds",
        "replicas": str(random.choice([3, 5, 7, 10])),
        "pid": str(random.randint(1000, 99999)),
        "process_name": random.choice(["java", "node", "python3", "gunicorn", "ruby"]),
        ...
        "go_error": random.choice(GO_ERRORS),
        "new_port": str(random.choice([8080, 8081, 8443, 9090])),
        "timeout": str(random.choice([5, 10, 30, 60])),
        "dns_server": f"10.{random.randint(0,255)}.{random.randint(0,255)}.{random.randint(1,254)}",
    ...
    incident: dict[str, Any] = {
        ...
        "affected_services": list(set([service] + random.sample(
            template["services"],
            k=min(2, len(template["services"]) - 1)
        ))),
        "affected_pods": [pod],
        "source": "synthetic",
        "timestamp": timestamp,
