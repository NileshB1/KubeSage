...
KubeSage DB Schema
==================
SQLAlchemy ORM schemas for tracking historical Kubernetes incidents, 
generated diagnostic reports, embedding mappings, and experiment runs.

Maintains relational persistence in Postgres.
"""
...
from sqlalchemy import (
    ...
    JSON, ForeignKey, Boolean, create_engine, Index,
)
...
class Incident(Base):
    ...
    # e.g., OOMKilled, CrashLoopBackOff, ImagePullBackOff, etc.
...
class EmbeddingMetadata(Base):
    """
    Stores embedding vector metadata (not the vector itself — ChromaDB handles that).
    Links embeddings to their source incidents.
    """
