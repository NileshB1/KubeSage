...
Handles system directories, embedding models, LLM parameters, and Postgres DB connections. 

Can be customized via a local .env file.
"""
...
class Settings(BaseSettings):
    ...
    ## TODO Alternatives: Qwen/Qwen2.5-1.5B-Instruct, Qwen/Qwen2.5-3B-Instruct
    ...
    LLM_TEMPERATURE: float = 0.1  # Low temperature for now
    LLM_DEVICE: str = "cpu" 
    ...
    RAG_TOP_K: int = 5
    RAG_SIMILARITY_THRESHOLD: float = 0.5 # Min cosine
    ...
    API_PORT: int = 8000
    API_CORS_ORIGINS: list[str] = ["http://localhost:8501"]  # Streamlit TODO for now using streamlit
...
for dir_attr in [
    "DATASETS_DIR","EMBEDDINGS_DIR", "VECTOR_DB_DIR","MODELS_DIR", 
