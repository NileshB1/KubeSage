"""
KubeSage Embedding Generator
============================
Encodes cleaned Kubernetes incident descriptions and log contents into 
dense numerical vector spaces using Sentence-Transformers (SBERT). 
Defaults to all-MiniLM-L6-v2 but supports BAAI/bge-base-en-v1.5 and e5-base.
"""
...
def prepare_texts_from_incidents(incidents: list[dict[str, Any]]) -> list[str]:
    ...
    Uses the 'cleaned_text' field if available (preprocessed),
    otherwise falls back to building text from raw fields.
...
def main() -> None:
    ...
    print(f"\n[OK] Generated {len(embeddings)} embeddings ({generator.dimension}-dim)")
